...
# --- устанавливаем зависимости Ruby и системные библиотеки ---
RUN apk --update add --no-cache \
    build-base \
    yaml-dev \
    tzdata \
...
# --- копируем уже собранный фронтенд в контейнер ---
# Убедись, что локально / CI собрал frontend в public/vite
# docker cp public/vite/. <container>:/app/public/vite
# или через git если разрешено
COPY public/vite ./public/vite

# --- экспонируем порт Rails ---
EXPOSE 3000
